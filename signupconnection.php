    ...
    if (empty($username) || empty($password) || empty($repeatPassword) || empty($lastname) || empty($firstname) || empty($cellphone) || empty($address)) {
        ...
        if ($password !== $repeatPassword) {
            ...
            if ($result->num_rows > 0) {
                ...
                $sql = "INSERT INTO users (username, password, lastname, firstname, middlename, cellphone, address,role, `student_number`)
                        VALUES ('$username', '$hashedPassword', '$lastname', '$firstname', '$middlename', '$cellphone' ,'$address','$portal', '$studentnum')";
